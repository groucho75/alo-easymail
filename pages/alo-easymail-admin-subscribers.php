	...
	<tr>
		...
		if ( $alo_em_cf ) {
			foreach( $alo_em_cf as $key => $value ){
				...
				?>
				<th scope="col" class="subscriber-custom--<?php echo $key ?>"><?php
		...
		?>
		<th scope="col" class="subscriber-username"><?php echo __("Username", "alo-easymail") . alo_em_help_tooltip( __("The username of registered users. It is blank for public subscribers.", "alo-easymail") ) ?></th>
		<th scope="col" class="subscriber-joindate"><?php echo "<a href='".$link_string."&amp;sortby=join_date".( ( isset($_GET['order']) && $_GET['order'] == 'DESC' )? "&amp;order=ASC": "&amp;order=DESC")."' title='".esc_attr(__("Order by join date", "alo-easymail"))."'>".__("Join date", "alo-easymail")."</a>"; ?></th>
		<th scope="col" class="subscriber-lastact"><?php echo "<a href='".$link_string."&amp;sortby=last_act".( ( isset($_GET['order']) && $_GET['order'] == 'DESC' )? "&amp;order=ASC": "&amp;order=DESC")."' title='".esc_attr(__("Order by date of last activity", "alo-easymail"))."'>".__("Last activity", "alo-easymail")."</a>" . alo_em_help_tooltip( __("The most recent date when the subcriber made an action", "alo-easymail") ." (". __("e.g. open a newsletter, click a newsletter link, update info in subscription form", "alo-easymail").")" ); ?></th>
		<th scope="col" class="row-important-column subscriber-active"><?php echo "<a href='".$link_string."&amp;sortby=active".( ( isset($_GET['order']) && $_GET['order'] == 'DESC' )? "&amp;order=ASC": "&amp;order=DESC")."' title='".esc_attr(__("Order by activation state", "alo-easymail"))."'>".__("Activated", "alo-easymail")."</a>" .alo_em_help_tooltip( __("For registered users the dafault state is activated. For public subscribers the default state is deactivated: it will be activated by clicking on the activation link in the e-mail.", "alo-easymail") ." ". __("A subscriber will be deleted if not activated in 5 days.", "alo-easymail") ); ?></th>
		<th scope="col" class="subscriber-lists"><?php _e("Mailing Lists", "alo-easymail"); ?></th>
		<th scope="col" class="subscriber-lang"><?php echo "<a href='".$link_string."&amp;sortby=lang".( ( isset($_GET['order']) && $_GET['order'] == 'ASC' )? "&amp;order=DESC": "&amp;order=ASC")."' title='".esc_attr(__("Order by language", "alo-easymail"))."'>".__("Language", "alo-easymail")."</a>"; ?></th>
		<th scope="col" class="row-important-column subscriber-actions"><?php _e("Actions", "alo-easymail") ?></th>
	</tr>
	</thead>
